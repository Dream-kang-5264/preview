...
import { Button, GetProps, Input, Table, Col, Row, TableProps, TableColumnsType, Form, FormProps, Modal, message } from 'antd';
const { Search } = Input;
import { CheckCircleOutlined, DeleteOutlined, DownloadOutlined, PlusOutlined } from '@ant-design/icons';
import { addDutyList, deleteLexicon, getDutyList } from '@/api/Dictionary';
...
const JobComparison: React.FC = () => {
    ...
    let [JobComparisonData, setJobComparisonData] = useState<any>({ currPage: 1, pageSize: 10, total: 0, currData: [], allData: [], checkedList: [], modalOpen: false, search_title: '' })
    let addRorm = useRef<any>(null)

    let handleJobComparisonList = (page: number, pageSize: number, value: string) => {
        getDutyList({ page: page, limit: pageSize, search_title: value }).then((res: any) => {
    ...
    useEffect(() => {
        ...
        handleJobComparisonList(JobComparisonData.currPage, JobComparisonData.pageSize, JobComparisonData.search_title)
    ...
    let handleOnChange = (page: number, pageSize: number) => {
        ...
        handleJobComparisonList(page, pageSize, JobComparisonData.search_title)
    }
    // 确定新增
    const handleOk = () => {
        if (addRorm.current) {
            addRorm.current?.validateFields().then((values: any) => {
                addDutyList({ ...values }).then((res) => {
    ...
    const onSearch: SearchProps['onSearch'] = (value, _e, info) => {
        ...
        handleJobComparisonList(JobComparisonData.currPage, JobComparisonData.pageSize, value)
        setJobComparisonData((prev: any) => ({
            ...prev,
            search_title: '',
        }))
    ...
    let handleDeletes = () => {
        Modal.confirm({
            ...
            onOk() {
                deleteLexicon({ ids: JobComparisonData.checkedList, type: 'type_2' }).then((res) => {
                    if (res.data.status.code === 200) {
    ...
    return (
        ...
        >
            <Row style={{ marginBottom: '10px' }}>
                <Col span={6}>
                    <Search placeholder="请输入关键字"
                        value={JobComparisonData.search_title}
                        onSearch={onSearch}
                        onChange={(e) => {
                            setJobComparisonData((prev: any) => ({
                                ...prev,
                ...
                <Col span={4} push={14} style={{ display: 'flex', justifyContent: 'space-between' }}>
                    <Button type='primary' onClick={handleAddJobComparison}><PlusOutlined />新增</Button>
                    {/* <Button><CheckCircleOutlined />已应用</Button>
                    <Button><DownloadOutlined />导入</Button> */}
                    <Button onClick={handleDeletes}
                        disabled={JobComparisonData.checkedList.length === 0}><DeleteOutlined />批量删除</Button>
                </Col>
            </Row>

            <NewTable
                tableData={JobComparisonData.allData}
                tableColumns={columns}
                tableCurrPage={JobComparisonData.currPage}
                tablePageSize={JobComparisonData.pageSize}
                tableTotal={JobComparisonData.total}
            ...
            <NewModal
                modalTitle={'新增落马官员'}
                modalOpen={JobComparisonData.modalOpen}
                handleCancel={handleCancel}
                handleOk={handleOk}
                modalForm={JobComparisonForm}
            />
        </PageContainer>
    );
};
