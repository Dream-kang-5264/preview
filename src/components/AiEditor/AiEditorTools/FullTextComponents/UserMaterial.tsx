...
function UserMaterial({ sendSelectId, setSendServer }: any) {
    // 个人素材
    ...
    // 获取所有素材
    let handleGetallMaterial = (type: any, page?: any) => {


        getAllUploadMaterial({ material_type: type, page: page, }).then((res) => {
            const data: DataType[] = [];
    ...
    const updateCombinedSelectedKeys = (firstSet: React.Key[], secondSet: React.Key[], thirdSet: React.Key[]) => {
        ...
        setSendServer((prev: any) => ({
            ...prev,
            material_id: [...new Set([...prev.material_id, ...combinedSelectedKeys])]
        }));

        sendSelectId(arr); // 发送合并后的数据
    };
