import { delUploadMaterial, getAllUploadMaterial, modifyMaterialPermission, uploadFileImg, userAffixText, userUploadDocument, userUploadPdf } from '@/api/search';
...
function indexs() {
    ...
    // 开关变化
    let onSwitchChange = (item: any) => {
        modifyMaterialPermission({ uploaded_files_id: item.id }).then((res) => {
            console.log(res);
        ...
        // console.log('开关变化', item);
    }
    // 获取全部
    let getUploadData = (type: any, title?: string) => {
        getAllUploadMaterial({ material_type: type, search_title: title }).then((res) => {
            ...
            if (res.status === 200) {
                ...
                res.data.data.data.forEach((item: any) => {
                    data.push({
                        ...
                        title: item.title,
                        authority: < div><Switch defaultChecked={item.visibility
                            === 'public' ? false : true} onChange={() => { onSwitchChange(item) }} /> 私密</div >
                    })
                })
    ...
    let handleOnChange = (page: any, pageSize: any) => {
        getAllUploadMaterial({ page: page, limit: pageSize, material_type: uploadData.currTable }).then((res) => {
            const data: DataType[] = [];
